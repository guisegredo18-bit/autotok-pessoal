...
declare global {
  ...
  var __granaSql: ReturnType<typeof postgres> | undefined;
}
...
function getDb(): Db {
  ...
  const sql = globalThis.__granaSql ?? postgres(env.databaseUrl, { max: 1, prepare: false });
  if (process.env.NODE_ENV !== 'production') globalThis.__granaSql = sql;
